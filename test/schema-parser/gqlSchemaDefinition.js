...
const schemaParser = require('../../lib/schema-parser/schemaParser')
const Util = require("util");

const schemaCode = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'schema.gql'),
    'utf8'
)
const studentSchema = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'student.gql'),
    'utf8'
)
const schoolSchema = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'school.gql'),
    'utf8'
)
const invalidSchemaDefinition = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'invalidSchemaDefinition.gql'),
    'utf8'
)
const invalidSchemaTypeName = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'invalidSchemaTypeName.gql'),
    'utf8'
)
const multiplesOperations = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'multiplesOperations.gql'),
    'utf8'
)
const missingOperation = fs.readFileSync(
    path.join(__dirname, '../resources/schema', 'missingOperation.gql'),
    'utf8'
)
  ...
  describe('Type Me', () => {
    ...
    it('Schema should have the properties with the null type and array type', () => {
      ...
      expect(schema.Me.fields).to.have.deep.include({
        ...
        directives: [{
          name: "deprecated",
          args: [{name: 'reason', value: 'Result field'}]
        }],
        noNullArrayValues: false,
        isDeprecated: true,
      })
