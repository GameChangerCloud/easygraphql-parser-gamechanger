 ...
 * @returns
 */
export const getRelations = (types, scalarTypeNames, env) => { // console.log(JSON.stringify(types,null, 3), "\n",typenames, "\n", scalarTypeNames)
            ...
            rfields.forEach(rfield => { // Check if it's not a scalar type
                ...
                if( !rfield["delegated_field"]["state"]){
                    ...
                    if (out == 2 && inn == 2) { // Checking if self join (type related to itself)
                        if (rfield.type === type.typeName) {
                            ...
                            rfield["joinTable"]["contains"].push({
                                ...
                                "type" : rfield.type,
                                "constraint" : "FOREIGN KEY (\""+rfield.name.toLowerCase()+"_id\") REFERENCES \"" + getSQLTableName(rfield.type) + "\" (\"Pk_" + getSQLTableName(rfield.type) + "_id\")"
                            ...
                            rfield["joinTable"]["contains"].push({
                                ...
                                "type" : rfield.type,
                                "constraint" : "FOREIGN KEY (\""+rfield.type.toLowerCase()+"_id\") REFERENCES \"" + getSQLTableName(rfield.type) + "\" (\"Pk_" + getSQLTableName(rfield.type) + "_id\")"
                            ...
                            if (rfield.directives.length>0 && rfield.directives.filter(directive => directive.name == 'hasInverse').length >0) {
                                ...
                                rfield["joinTable"]["contains"].push({
                                    "fieldName" : rfield.name.toLowerCase(),
                                    "type" : rfield.type,
                                    "constraint" : "FOREIGN KEY (\""+rfield.name+"_id\") REFERENCES \"" + getSQLTableName(rfield.type) + "\" (\"Pk_" + getSQLTableName(rfield.type) + "_id\")"
                        ...
                        // if field can be null, then we do classic processing
                            ...
                            // delegated field is a foreign Key
                            delegatedField["foreign_key"] = {
                                "name": "Fk_"+rfield.name+"_"+getSQLTableName(rfield.type)+"_id",
                                "type": "int",
                                "noNull": rfield.noNull,
                                "isArray": false,
                                "foreignKey": true,
                                "constraint": "FOREIGN KEY (\"Fk_"+rfield.name+"_"+getSQLTableName(rfield.type)+"_id\") REFERENCES \"" + type.sqlTypeName + "\" (\"Pk_" + type.sqlTypeName + "_id\")"

                            }
                            delegatedField["delegated_field"]["state"] = true
                            delegatedField["name"] = "Fk_"+rfield.name+"_"+getSQLTableName(rfield.type)+"_id",
                                delegatedField["type"] = "Int"
                        ...
                        if (rfield.type === type.typeName) {
                            ...
                            rfield["foreign_key"] = {
                                ...
                                "isArray": false,
                                "foreignKey": true,
                                "constraint": "FOREIGN KEY (\"Fk_"+rfield.name+"_"+getSQLTableName(rfield.type)+"_id\") REFERENCES \"" + getSQLTableName(rfield.type) + "\" (\"Pk_" + getSQLTableName(rfield.type) + "_id\")"
                            }
                    ...
                    // One only
                        ...
                        let targetSQLTypeName = getSQLTableName(rfield.type)
                        rfield["foreign_key"] = {
 ...
 */
export const getJoinTables = (types, scalarTypeNames) => {
    let result :any = []
